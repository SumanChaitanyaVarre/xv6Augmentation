...
int alloc_inode(int mount_point)
{
	...
	*/
	if (mount_point < 0 || mount_point >= 10)
	{
		printf("Overflow in alloc inode");
	}
	struct superblock_t superblock;
	int inode_number;
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
	...
	memcpy(inodeptr, tempBuf + inodenum * sizeof(struct inode_t), sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr)
{
	if (mount_point < 0 || mount_point >= 10)
	{
		// printf("Overflow in write inode \n");
		int i;
		// return;
	}
	/*
	...
	inodeptr1[inodenum] = *inodeptr;
	// printf("Inside write inode %d \n", mounts[mount_point].device_fd);
	writeblock(mounts[mount_point].device_fd, inode_block, tempBuf);

	// int block = inodenum / 16 + 1;
	// char tempBuf[BLOCKSIZE];
	// readblock(mounts[mount_point].device_fd, block, tempBuf);
	// memcmp(tempBuf + (inodenum % 16) * (sizeof(struct inode_t)), inodeptr, sizeof(struct inode_t));
	// // tempBuf[inodenum % 16] = inodeptr;
	// writeblock(mounts[mount_point].device_fd, block, tempBuf);
}
...
void write_datablock(int mount_point, int blocknum, char *buf)
{
	 ...
	 */
	// encrypt the block if its an encrypted system
